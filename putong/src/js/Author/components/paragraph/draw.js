import {
    ...
    useEndStyle, useBranchStyle, useLinkStyle, useInversedTextStyle, useDisabledTextStyle, useLockStyle,useGoOnStyle,
} from './styles';
...
export function drawLock(element, style) {
    ...
    const erroricon = imagePatterns['erroricon'];
    const warningicon = imagePatterns['warningicon'];
    const lockicon = imagePatterns['lockicon'];
    const left = x - nodeHalfWidth;
    const right = x + nodeHalfWidth;
    const top = y - nodeHalfHeight;
    const bottom = y + nodeHalfHeight;
    ...
    if (isInBoundingBox(left, right, top, bottom)) {
        ...
        //绘制锁图标
        if (lockicon && lockicon.image) {
            context.save();
            context.drawImage(lockicon.image, x - 65, y - 10, 20, 20);
            context.restore();
        }
        ...
        context.fillText(title.length > 12 ? (title.substring(0, 12) + '...') : title, x + 10, y);
        context.restore();
    }
...
export function drawGoOn(element, style) {
    const { hasError, hasWarning, hasComment, disabled } = style;
    let { x, y, title } = element;
    const erroricon = imagePatterns['erroricon'];
    const warningicon = imagePatterns['warningicon'];
    const goonicon = imagePatterns['goonicon'];
    const left = x - nodeHalfWidth;
    const right = x + nodeHalfWidth;
    const top = y - nodeHalfHeight;
    const bottom = y + nodeHalfHeight;
    ...
    if (isInBoundingBox(left, right, top, bottom)) {
        ...
        //绘制待续图标
        if (goonicon && goonicon.image) {
            context.save();
            context.drawImage(goonicon.image, x - 65, y - 10, 20, 20);
            context.restore();
        }
...
export function drawEnd(element, style) {
    ...
    const { disabled, hasComment } = style;

    const a = nodeHalfWidth;
    const b = nodeHalfHeight;
    const ox = 0.5 * a;
    const oy = 0.6 * b;
    const left = x - nodeHalfWidth;
    const right = x + nodeHalfWidth;
    ...
    if (isInBoundingBox(left, right, top, bottom)) {
        ...
        if (hasComment) {
            ...
            context.fillStyle = '#fff';
            context.beginPath();
            context.arc(x + 73, y - 12, 8, 0, Math.PI * 2, true);
            context.closePath();
            context.fill();
            context.restore();
            context.save();
            context.fillStyle = '#f76b6b';
            context.beginPath();
            context.arc(x + 73, y - 12, 5, 0, Math.PI * 2, true);
            context.closePath();
            context.fill();
            context.restore();
        }
